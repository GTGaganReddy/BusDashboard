// Setup permanent database with May 1st assignments
...
async function setupPermanentDatabase() {
  console.log('Setting up permanent database...');
  
  // Create new database connection
  const pool = new Pool({
    connectionString: 'postgresql://replit:replit@db.thin.replit.com:5432/main?sslmode=require'
  });
  ...
  try {
    ...
    `;
    await client.query(driversQuery);
    ...
    `;
    await client.query(routesQuery);
    
    // Insert May 1st assignments
    console.log('Inserting May 1st assignments...');
    const assignmentsQuery = `
      INSERT INTO assignments (
        assigned_date, driver_name, route_number, route_description, 
        route_hours, driver_hours_remaining, status
      ) VALUES
    ...
    `;
    await client.query(assignmentsQuery);
    
    console.log('✅ Database setup completed successfully!');
    console.log('- 21 drivers inserted');
    console.log('- 5 routes inserted');
    console.log('- 5 May 1st assignments inserted');
